%CYG_ROOT%\setup-x86.exe -q -P ocaml,ocaml-camlp4,ocaml-compiler-libs,libncurses-devel,unzip,libmpfr-devel,patch,flexdll
...
%CYG_BASH% "cd /cygdrive/c/projects/opam-full-1.2.2 && env DJDIR=workaround ./configure && make lib-ext && make && make install"
%CYG_BASH% "opam init -ya"
%CYG_BASH% "opam switch %COMPILER%"
...
%CYG_BASH% "opam pin add -yn oasis /cygdrive/c/projects/oasis"
cd ../lwt
